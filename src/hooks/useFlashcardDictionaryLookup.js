...
export const parseDictionaryLookupResponse = (
  data,
  ...
  const pronunciation =
    Array.isArray(firstResult.pronunciations) &&
...
const useFlashcardDictionaryLookup = (items, setItems) => {
  ...
  useEffect(() => {
    return () => {
      Object.values(lookupTimeoutsRef.current).forEach((timeoutId) =>
  ...
  const handleLookupFromDictionary = useCallback(
    async (index, vocabOverride) => {
      const vocab = (
        vocabOverride ??
        itemsRef.current[index]?.vocabulary ??
        ""
      ).trim();
      if (!vocab) {
      ...
      const currentDefinition = (
        itemsRef.current[index]?.definition ?? ""
      ).trim();
      ...
      try {
        ...
        const currentVocab = (itemsRef.current[index]?.vocabulary ?? "")
          .trim()
          .toLowerCase();
  ...
  const cleanupLookupForIndex = useCallback(
    (index) => {
      const existing = lookupTimeoutsRef.current[index];
      if (existing) {
        clearTimeout(existing);
