...
function shouldPreferWritingStatisticsApi(data) {
  ...
  if (
    ...
    Object.keys(data.userAnswersByPart).some((k) => /writing/i.test(String(k)))
  ) {
    return true;
  }
  return false;
...
function shouldPreferSpeakingStatisticsApi(data) {
  ...
  if (
    data.userAnswersByPart &&
    Object.keys(data.userAnswersByPart).some((k) => /speaking/i.test(String(k)))
  ) {
    return true;
  }
  return false;
...
const StatisticalResults = ({ userTestId, testId, onViewAnswers }) => {
  ...
  const handleLearningPathClick = async () => {
    ...
    try {
      ...
      message.error(
        error?.message || "Unable to load learning path information",
      );
    } finally {
  ...
  const incorrectAnswers = useMemo(() => {
    if (!data) return 0;
    if (data.isWritingSummary) {
      return Math.max(0, (data.totalQuestions ?? 0) - (data.totalAnswers ?? 0));
    }
    if (fullTest) {
  ...
  const currentPartDetails = useMemo(() => {
    if (
      ...
      !data ||
      !data.userAnswersByPart ||
      !data.userAnswersByPart[activePartTab]
    ) {
      return null;
    }
    ...
    const categories = partData
      ...
      .map((item) => {
        ...
        const questions =
          item.userAnswerOverallResponses?.map((q) => ({
            position: q.position,
            correct: q.correct,
  ...
  return (
    <div className="relative bg-white rounded-xl border border-gray-200 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
          <div className="text-sm text-gray-600 mb-1">
            {data.isWritingSummary ? "Completion rate" : "Accuracy"}
          </div>
          <div className="text-2xl font-bold text-gray-900">
      ...
      {/* Key Metrics */}
      <div
        className={`grid gap-4 mb-6 ${fullTest ? "grid-cols-2 md:grid-cols-3" : "grid-cols-2 md:grid-cols-2"}`}
      >
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xl font-bold text-gray-900">
              {data.correctAnswers || 0} questions
            </div>
          </div>
        </div>
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 flex items-center gap-3">
          ...
          <div>
            <div className="text-sm text-gray-600">
              {data.isWritingSummary ? "Unanswered" : "Wrong answers"}
            </div>
            <div className="text-xl font-bold text-gray-900">
      ...
      {parts.length > 0 && !data.isWritingSummary && (
        <div className="border-t border-gray-200 pt-6">
          ...
          {/* Table */}
          {!currentPartDetails ||
          !currentPartDetails.categories ||
          currentPartDetails.categories.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              {activePartTab
                ? `No data for ${activePartTab}`
