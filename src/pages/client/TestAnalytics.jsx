...
const getAccuracyColorClass = (accuracy) => {
  if (accuracy >= 70) {
    ...
    return "font-medium text-rose-500";
  }
};
...
const Card = ({ title, value, subtitle, highlight }) => (
  <div className="flex-1 min-w-[160px] rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
    ...
    {subtitle && (
      <div
        className={`mt-1 text-xs ${highlight ? "text-blue-600 font-medium" : "text-gray-500"}`}
      >
        {subtitle}
      </div>
    )}
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-gradient-to-br from-gray-900 to-gray-800 text-white text-sm rounded-xl px-4 py-3 shadow-2xl border border-gray-700 backdrop-blur-sm">
          <div className="font-bold mb-3 pb-2 border-b border-gray-600 text-base">
            {label}
          </div>
    ...
    Object.keys(partStats).forEach((partName) => {
      if (partCounts[partName] > 0) {
        result.push({
          part: partName,
          value:
            Math.round((partStats[partName] / partCounts[partName]) * 100) /
            100,
        });
      }
    });
...
const TestAnalytics = () => {
  ...
  const startRow =
    totalRows === 0 ? 0 : historyMeta.page * effectivePageSize + 1;
  const endRow =
    totalRows === 0
      ? 0
      : Math.min((historyMeta.page + 1) * effectivePageSize, totalRows);

  // Get current exam data based on activeTab
  const currentExam =
    analyticsData?.examList?.[activeTab === "listening" ? 0 : 1];
  ...
  const apiAccuracy =
    currentExam?.correctPercent != null
      ? Number(currentExam.correctPercent)
      : null;
  ...
  const availableParts = currentExam?.userAnswersByPart
    ? Object.keys(currentExam.userAnswersByPart)
    : [];
  ...
  useEffect(() => {
    if (analyticsData?.examList) {
      ...
      if (exam?.userAnswersByPart) {
        const parts = Object.keys(exam.userAnswersByPart).filter(
          (p) => p !== "Total",
        );
        if (parts.length > 0) {
  ...
  const hasScoreData = Boolean(
    scoreStats && (scoreStats.chartData?.length ?? 0) > 0,
  );
  ...
  const handleCreateMiniTest = async () => {
    ...
    try {
      const response = await createMiniTest(
        miniTestPartId,
        selectedTags,
        numberOfQuestions,
      );
      ...
      message.error(
        error?.response?.data?.message || "Unable to create mini test",
      );
    } finally {
      ...
      <div className="max-w-6xl mx-auto px-4 space-y-8">
        <div className="rounded-3xl bg-gradient-to-b from-indigo-50 to-white border border-indigo-100 p-8 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3 text-indigo-700 mb-2 sm:mb-0">
            ...
            <div>
              <p className="text-sm tracking-wide uppercase font-semibold text-indigo-500">
                TOEIC
              </p>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          {analyticsLoading ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              <Card
                title="Test attempts"
                value={analyticsData?.numberOfSubmissions || 0}
                subtitle="attempts"
              />
              <Card
                title="Completed tests"
                value={analyticsData?.numberOfTests || 0}
                subtitle="tests"
              />
              <Card
                title="Practice time"
                value={secondsToMinutes(analyticsData?.totalTimes || 0)}
                subtitle="minutes"
              />
              <Card
                title="Target score"
            ...
            <Card
              ...
              subtitle={
                currentStats.totalQuestions > 0
                  ? `${currentStats.correctAnswers || 0}/${currentStats.totalQuestions} correct`
                  : "No data yet"
              }
            />
          </div>
                    ...
                    {analyticsLoading ? (
                      ...
                      currentPartData.map((item, idx) => {
                        ...
                        return (
                          ...
                          >
                            <td
                              className={`px-4 py-3 ${isTotal ? "text-blue-900 font-bold" : "text-gray-700"}`}
                            >
                              {item.tag}
                            </td>
                            <td
                              className={`px-4 py-3 text-center ${isTotal ? "text-blue-900 font-bold" : "font-semibold text-gray-900"}`}
                            >
                              {item.correctAnswers || 0}
                            </td>
                            <td
                              className={`px-4 py-3 text-center ${isTotal ? "text-blue-800 font-semibold" : "text-gray-600"}`}
                            >
                              {item.wrongAnswers || 0}
                            </td>
                            <td
                              className={`px-4 py-3 text-center ${isTotal ? "text-blue-900 font-bold" : getAccuracyColorClass(displayPercent)}`}
                            >
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  ...
                  <p className="text-sm text-gray-600">
                    Create a small test to practice the topics you want to
                    improve
                  </p>
                </div>
              </div>
            ...
            <>
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-6">
                <Card
                  ...
                  value={
                    scoreStats.averageScore
                      ? Math.round(scoreStats.averageScore)
                      : 0
                  }
                  subtitle={`/990`}
                />
                ...
                <Card
                  ...
                  value={
                    scoreStats.averageListeningScore
                      ? Math.round(scoreStats.averageListeningScore)
                      : 0
                  }
                ...
                <Card
                  ...
                  value={
                    scoreStats.averageReadingScore
                      ? Math.round(scoreStats.averageReadingScore)
                      : 0
                  }
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="text-sm text-gray-600">
              Showing{" "}
              <span className="font-semibold text-gray-900">{startRow}</span> -{" "}
              <span className="font-semibold text-gray-900">{endRow}</span> of{" "}
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() =>
                  setHistoryPage((page) => Math.min(totalPages, page + 1))
                }
