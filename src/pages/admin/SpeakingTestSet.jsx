...
const SpeakingTestSetPage = () => {
    ...
    const isFetching = useAppSelector(state => state.speakingTestSets.isFetching);
    const meta = useAppSelector(state => state.speakingTestSets.meta);
    const testSets = useAppSelector(state => state.speakingTestSets.result);
    ...
    const handleShowTests = async (testSet) => {
        ...
        try {
            ...
            const response = await getTestInSpeakingTestSet(testSet.id, query);
    ...
    const handleFetchTests = async (query) => {
        ...
        setLoadingDrawer(true);
        try {
            const response = await getTestInSpeakingTestSet(currentTestSet.id, query);
            setTestData(response.data);
    ...
    return (
        <div>
            <DataTable
                actionRef={tableRef}
                formRef={formRef}
                headerTitle="Speaking Test Sets"
                rowKey="id"
                loading={isFetching}
                columns={columns}
                dataSource={testSets}
                ...
                request={async (params, sort, filter) => {
                    ...
                    dispatch(fetchSpeakingTestSets({ query }));
                }}
                scroll={{ x: true }}
                pagination={{
                    current: meta.page + 1,
            ...
            <ModalTestSet
                ...
                createTestSet={createSpeakingTestSet}
                updateTestSet={updateSpeakingTestSet}
            />
