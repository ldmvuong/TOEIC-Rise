...
import { fetchWritingTests } from "../../redux/slices/writingTestManagementSlide";
...
import { importWritingTests, getAllWritingTestSets } from "../../api/api";
...
const WritingTestPage = () => {
  ...
  const isFetching = useAppSelector((state) => state.writingTests.isFetching);
  const meta = useAppSelector((state) => state.writingTests.meta);
  const tests = useAppSelector((state) => state.writingTests.result);
      ...
      render: (_value, entity) => (
        <Space>
          <FundViewOutlined
            ...
            title="View details"
            onClick={() => {
              navigate(`/admin/writing-tests/${entity.id}`);
            }}
          />
        </Space>
      ),
    ...
    <div>
      <DataTable
        ...
        request={async (params, sort, filter) => {
          ...
          dispatch(fetchWritingTests({ query }));
        }}
        scroll={{ x: true }}
        pagination={{
          current: meta.page + 1,
