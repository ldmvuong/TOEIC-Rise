...
import { getWritingTestById, updateWritingTest } from "@/api/api";
...
import WritingPartDetail from "@/components/admin/test-detail/WritingPartDetail";
...
const WritingTestDetailPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchDetail = async () => {
            ...
            setError("");
            try {
                const res = await getWritingTestById(id);
                if (!isMounted) return;
    ...
    return (
        <div className="p-4 space-y-6 max-w-7xl mx-auto">
            ...
            />
            <WritingPartDetail part={selectedPart} />
            <EditTestNameModal
                open={openEditName}
                onClose={() => setOpenEditName(false)}
                test={test}
                onSuccess={reloadTest}
                updateTestFn={(tid, payload) => updateWritingTest(tid, payload)}
            />
...
export default WritingTestDetailPage;
