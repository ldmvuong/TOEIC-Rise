...
import { getSpeakingTestById, updateSpeakingTest } from "@/api/api";
...
import SpeakingPartDetail from "@/components/admin/test-detail/SpeakingPartDetail";
...
const SpeakingTestDetailPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchDetail = async () => {
            ...
            setError("");
            try {
                const res = await getSpeakingTestById(id);
                if (!isMounted) return;
    ...
    if (error) {
        return (
            <div className="p-4">
                ...
                <button type="button" className="px-3 py-2 bg-gray-200 rounded" onClick={() => navigate(-1)}>Go back</button>
            </div>
        );
    }
    ...
    return (
        <div className="p-4 space-y-6 max-w-7xl mx-auto">
            ...
            />
            <SpeakingPartDetail part={selectedPart} />
            <EditTestNameModal
                open={openEditName}
                onClose={() => setOpenEditName(false)}
                test={test}
                onSuccess={reloadTest}
                updateTestFn={(tid, payload) => updateSpeakingTest(tid, payload)}
            />
...
export default SpeakingTestDetailPage;
