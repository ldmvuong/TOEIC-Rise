...
const WritingTestSetPage = () => {
    ...
    const isFetching = useAppSelector(state => state.writingTestSets.isFetching);
    const meta = useAppSelector(state => state.writingTestSets.meta);
    const testSets = useAppSelector(state => state.writingTestSets.result);
    ...
    const handleShowTests = async (testSet) => {
        ...
        try {
            ...
            const response = await getTestInWritingTestSet(testSet.id, query);
    ...
    const handleFetchTests = async (query) => {
        ...
        setLoadingDrawer(true);
        try {
            const response = await getTestInWritingTestSet(currentTestSet.id, query);
            setTestData(response.data);
        ...
        <div>
            <DataTable
                ...
                request={async (params, sort, filter) => {
                    ...
                    dispatch(fetchWritingTestSets({ query }));
                }}
                scroll={{ x: true }}
                pagination={{
                    current: meta.page + 1,
            ...
            <ModalTestSet
                ...
                createTestSet={createWritingTestSet}
                updateTestSet={updateWritingTestSet}
            />
...
export default WritingTestSetPage;
